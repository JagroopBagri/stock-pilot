...
export default function ResetPasswordPage() {
  ...
  const onUpdatePasswordSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      await axios.post("/api/v1/users/reset-password", { token: resetToken, password });
      toast.success("Your password has been updated successfully.");
      setPassword("");
      router.push("/login");
    } catch (error: any) {
      toast.error("Failed to update password. This link may have expired. Please request another reset-password email and try again.");
